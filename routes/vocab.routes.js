...
try {
   ...
   words = await Word.find({ trainDate: { $gte: _start, $lte: _end } }).sort( {train1:1,"trainDate" : 1} )
...
router.get('/', auth, async (req, res) => {
  ...
  try {
    if((nav=='null')&&(navWord=='null')){
      console.log('first')
     word = await Word.findOne().sort( {"trainDate" : 1} )
   // console.log('word=',word)
    }else if((nav=='prev')&&(navWord!='null')){
     
       word = await Word.findOne({ trainDate: { $lt: navWord } }).sort( {"trainDate" : -1} )
       if(word===null){
        word = await Word.findOne().sort( {"trainDate" : -1} ) 
       } 
      ...
      // console.log('countPrev=',countPrev)
        ...
        word = await Word.findOne({ trainDate: { $gt: navWord } }).sort( {"trainDate" : 1} ) 
        if(word===null){
        word = await Word.findOne({ trainDate: { $gte: navWord } }).sort( {"trainDate" : 1} ) 
         } 
    ...
    const wordAll = await Word.find({ trainDate: { $gte: _start, $lte: _end } }).countDocuments()
